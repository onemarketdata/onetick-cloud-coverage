# Exchanges -  Cash

Regional & National Stock Exchanges typically covering Equities & ETFs

It covers:

* `ABU_DHABI` - Abu Dhabi Securities Exchange
* `ALPHA` - TSX Alpha
* `ALPHA_X` - TSX Alpha-X / Alpha DRK
* `AMEX` - NYSE American Equities
* `AQSE` - Aquis Stock Exchange
* `AQUIS` - Aquis UK
* `AQUIS_EU` - Aquis Europe
* `ARCA_IMB` - ARCA Auction Imbalance Data
* `ARCA` - NYSE Arca Equities
* `ASX` - Australian Stock Exchange
* `ATHENS` - Athens Stock Exchange
* `BIVA` - BIVA (Bolsa Institucional de Valores)
* `BLUE_OCEAN` - Blue Ocean ATS
* `BRAZIL_EQ` - B3 (Bovespa) - Equities + Equity Options
* `BRAZIL_OTC` - B3 (Bovespa) - OTC Market
* `BSE` - BSE (Bombay Stock Exchange)
* `BUCHAREST` - Bucharest Stock Exchange
* `BUDAPEST` - Budapest Stock Exchange
* `BXE` - Cboe BXE (BATS Europe)
* `BYX` - Cboe BYX
* `BZX` - Cboe BZX Equities
* `CBOE_APA` - Cboe Europe Trade Reporting
* `CBOE_AU` - Cboe Australia (CXA)
* `CBOE_JP` - Cboe Japan
* `COLOMBIA` - Colombia Stock Exchange
* `CSE` - Canadian Securities Exchange
* `CSE2` - CSE2 Exchange
* `CX2` - Nasdaq CX2
* `CXC` - Nasdaq CXC
* `CXE` - Cboe CXE (Chi-X Europe)
* `CYPRUS` - Cyprus Stock Exchange
* `DE_REGIONAL` - German Regional Exchanges (Berlin, Munich, Dusseldorf, Hamburg, Hannover)
* `DFM` - Dubai Financial Market
* `DUBLIN` - Euronext Dublin (Irish Stock Exchange)
* `DXE` - Cboe DXE
* `EDGA` - Cboe EDGA
* `EDGX` - Cboe EDGX
* `EGYPT` - Egyptian Exchange
* `EQUIDUCT` - Equiduct Exchange
* `EURONEXT` - Euronext (Paris / Amsterdam / Brussels / Lisbon / ETFs + Funds / Best of Book)
* `FRANKFURT` - Frankfurt Stock Exchange
* `HANOI` - Hanoi Stock Exchange
* `HKSE` - Hong Kong Stock Exchange
* `HO_CHI_MINH` - Ho Chi Minh Stock Exchange
* `INDONESIA` - Indonesia Stock Exchange
* `ISTANBUL` - Borsa Istanbul - Equities
* `JSE` - Johannesburg Stock Exchange
* `KOREA_EQ` - Korea Exchange - Equities (KOSPI)
* `KOREA_ETF` - Korea Exchange - ETFs and ETNs
* `KOSDAQ` - Korea Exchange - Kosdaq
* `KUWAIT` - Kuwait Stock Exchange (Boursa Kuwait)
* `LIMA` - Lima Stock Exchange
* `LSE` - London Stock Exchange - UK Market
* `LSE_INT` - London Stock Exchange - International Market
* `LUXEMBOURG` - Luxembourg Stock Exchange
* `LYNX` - Lynx ATS
* `MADRID` - Madrid Stock Exchange (SIBE)
* `MALAYSIA` - Bursa Malaysia - Equities
* `MEXICO` - Mexico Stock Exchange (BMV)
* `MILAN` - Euronext Milan (Borsa Italiana) - Equities
* `NAGOYA` - Nagoya Stock Exchange
* `NASDAQ` - The Nasdaq
* `NASDAQ_DUBAI` - Nasdaq Dubai
* `NASDAQ_IMB` - Nasdaq Auction Imbalance Data
* `NASDAQ_PSX` - Nasdaq PSX
* `NASDAQ_TEXAS` - Nasdaq Texas
* `NEO` - Cboe Canada (formerly Neo Exchange)
* `NORDIC_EQ` - Nasdaq Nordic (Stockholm, Copenhagen, Helsinki, Iceland) - Equities
* `NSE_CASH` - National Stock Exchange of India - Equities
* `NSE_IX` - NSE International Exchange
* `NYSE` - New York Stock Exchange NYSE
* `NYSE_IMB` - NYSE Auction Imbalance Data
* `NYSE_TEXAS` - NYSE Texas
* `NZX` - New Zealand Exchange (NZX)
* `OMEGA` - Omega ATS
* `OSLO` - Euronext Oslo - Equities
* `OTCM` - OTC Markets (US OTC Quotes)
* `PRAGUE` - Prague Stock Exchange
* `PSE` - Philippine Stock Exchange
* `PURE` - CSE - TSX/Venture Listed (formerly Pure Trading)
* `QATAR` - Qatar Stock Exchange
* `SANTIAGO` - Santiago Stock Exchange
* `SAUDI` - Saudi Stock Exchange
